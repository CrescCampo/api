...
export default class InMemoryFarmerRepository implements FarmerRepository {
  ...
  findByGoogleId(googleId: string): Promise<Farmer | null> {
    const existingFarmer = this.items.find(user => user.googleId === googleId);
  ...
  findByIdForUpdate(id: string): Promise<Farmer | null> {
